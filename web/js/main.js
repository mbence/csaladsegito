...
JcsSettings = {
  ...
  setupRecFields: function() {
    return $(".admin-ct-tabs").tabs(".admin-panes > div", {
      "initialIndex": parseInt($("#form_act_tab").val()),
      "onClick": function() {
        return $("#form_act_tab").val(this.getIndex());
      }
    });
  }
};
...
JcsCatering = {
  ...
  initInvoices: function() {
    ...
    $(".catering-invoice").each(function() {
      if (3 === $(this).data("status")) {
        return $(this).click();
      }
    });
    return $(".invoice_cancel").on("click", function() {
      var id;
      if (confirm("Biztos benne?")) {
        id = $(this).data("id");
        $("#form_cancel_id").val(id);
        return $("#catering_form").submit();
      }
    });
  },
  initButtonRow: function() {
