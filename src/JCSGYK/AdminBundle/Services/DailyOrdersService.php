...
class DailyOrdersService
{
    ...
    public function run(OutputInterface $output = null)
    {
        ...
        // get the orders from the client order table
        ...
        $this->output(sprintf("%s: Megrendelések lekérdezve", date('H:i:s')));
        $order->setSummary($this->summary);
        $em->flush();
        ...
        if (!empty($exp)) {
            ...
            $order->setFiles($zip_file_contents);
            $order->setSummary($this->summary);
            $em->flush();
        ...
        $order->setSummary($this->summary);
        $order->setStatus(MonthlyClosing::SUCCESS);
        $em->flush();
